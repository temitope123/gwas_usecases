...
#tests to see if user inputs values.
if(!defined($affy_calls)||!defined($report)||!defined($annot)||!defined($out_dir)||!defined($out_name)){
print "Synopsis:\n";
...
my $map = "$out_dir" . "/mapfile.map"; #ouput map file
#my $map_plink_rs_id = "$out_dir" . "/plink_mapfile_rs_id.map"; #map file with rsid
my $map_plink_rs_id = "$out_dir" . "/$out_name.map"; #map file with rsid
my $map_plink_AFFY_id = "$out_dir" . "/plink_mapfile_AFFY_id.map"; # map file with AFFY id
#my $ped = "$out_dir" . "/pedfile.ped"; #output ped file
my $ped = "$out_dir" . "/$out_name.ped"; #output ped file
my $strandfile = "$out_dir" . "/strandfile.txt"; # output strand file
my $logfile = "$out_dir" . "/pedmaker.log.txt";  #log file
...
#open birdseed calls for reading
open(IN1,"<$affy_calls") || die print "Can't open recoded birdseed.calls.txt\n";

my @subjects;
my $total_subjects;

#read birseed file and get number of subjects
while (<IN1>){ 
if(m /^probeset_id/){
my $subjects = $_;
...
#creates hash map for annotation file
#uses the SNP id as keys
#values include an array ref of SNP id, chr number, strand, bp, alleles
my %snp_map;
while(<IN3>){
if((!m /^\#/) && (!m /^"Probe Set ID"/)){
...
#sets list separator to tab
$" = "\t";
#prints birdseed initial information and array id to file
while (<IN1>){
	if((m /^\#/) || (m /^probeset_id/)){
		print (OUT3 $_);
	}
...
#gets snps from calls files
#checks for it in annotation file
#prints values to map file
while (<IN1>){
	if(m/^(SNP\S+)\t/){
	my ($snp)=($1);
	...
	my $rsid = ($current->[0] eq "---" ? $snp : $current->[0]);
	my $chr = $current->[1];
	if ($chr ne "---") {
	my $pos = $current->[2];
	my $strand =$current->[3];
...
print (OUT1 "$chr\t$rsid\t0\t$pos\t$alleleA $alleleB\n"); #outputs to map file
print (OUT5 "$chr\t$rsid\t0\t$pos\n");#outputs to map_plink_rs_id
print (OUT6 "$chr $snp 0.00.1 $pos\n");#outputs to map_plink_AFFY_id
print (OUT2 "$chr\t$rsid\t$pos\t$strand\n");#ouputs to strand file
my @calls = (split /\t/, $_);
shift @calls;

map (s/-1/N\_N/g, @calls);#replace -1 with N_N
map (s/0/$alleleA\_$alleleA/g, @calls);#replace 0 with A_A
map (s/1/$alleleA\_$alleleB/g, @calls);#replace 1 with A_B
map (s/2/$alleleB\_$alleleB/g, @calls);#replace 2 with B_B
map (s/N\_N/0\_0/g, @calls);#replace N_N with 0_0

print (OUT3 "$snp\t@calls");}#end ne
...
#$g is the ids gotten from the calls file
#
if(m/^\Q$g\E/){my @line; #6_o
my @temp=split(/\t/, $_);
my $id = $temp[0];
$id = substr($id, 0, 10); #inidividual names is assigned here
my $sex = $temp[1];
